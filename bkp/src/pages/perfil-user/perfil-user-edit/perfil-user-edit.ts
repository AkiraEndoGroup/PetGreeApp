...
export class PerfilUserEditPage {
  ...
  async editAddress() {
    let modal = await this.modalCtrl.create(ModalEditAddress, {address: this.user.endereco})
    await modal.present()
  ...
  async editPhones() {
    let modal = await this.modalCtrl.create(ModalEditPhone, { phones: this.user.telefones })
    await modal.present()
